...
/*
 * Certains commentaires dans ce fichier font référence à la datasheet du composant.
...
void BSP_MCP23S17_demo(MCP23S17_direction_e direction)
{
	...
	while(1)
	{
		...
		{	// Démo pour tester les GPIO en output
			...
			// Cette démo n'a pas encore été testée
 ...
 * @param address: adresse du registre oÃ¹ envoyer la donnée
...
void BSP_MCP23S17_init(void)
{
	...
	if(!initialized)
	{
		...
		HAL_GPIO_WritePin(MCP23S17_RST_PORT, MCP23S17_RST_PIN, 0);
		HAL_Delay(10);
		HAL_GPIO_WritePin(MCP23S17_RST_PORT, MCP23S17_RST_PIN, 1);
		HAL_Delay(10);
...
{
	// On récupère l'octet contenant l'état des pins du port souhaité
...
/**
 * @brief 	Fonction pour contrôler la résistance pull-up d'un pin.
...
{
	// On récupère l'octet contenant l'état des résistances du port souhaité
...
/**
 * @brief 	Fonction pour contrôler les résistances pull-up des pins d'un port.
 * 			Si un bit est défini à 1 et que la broche correspondante est configurée en entrée,
 * 			la broche du port correspondante est tirée vers le haut en interne avec une résistance de 100 kOhm.
 * 			Plus d'infos page 22 de la datasheet.
...
{
	// On récupère l'octet contenant l'état des it du port souhaité
 ...
 *        Si le miroir est activé, les broches INTA et INTB sont "reliées" de sorte quâ€™une interruption sur lâ€™un ou lâ€™autre port provoquera lâ€™activation des deux broches.
 *        Si le miroir est désactivé, les broches INT sont "séparées". Les conditions dâ€™interruption sur un port entraÃ®neront lâ€™activation de sa broche INT respective.
 ...
 *        La polarité dâ€™interruption (INTPOL) définit la polarité de la broche INT.
 *        Ce bit nâ€™est fonctionnel que lorsque le bit ODR est effacé.
...
{
	// On récupère les états des valeurs par défaut
 ...
 *        Cette valeur par défaut va servir à déclencher des interruptions (voir page 20 de la datasheet).
 * @param port : port oÃ¹ vont Ãªtre appliquées les valeurs par défaut
 * @param state : valeur/état par défaut à appliquer
 */
...
/**
 * @brief Fonction qui permet de définir le contrôle d'interruption sur un pin
 * @param port : port du pin (MCP23S17_PORT_A ou MCP23S17_PORT_B)
 * @param pin : le pin sur lequel on souhaite définir le contrôle (voir MCP23S17_pin_e)
 * @param type : type de contrôle que l'on souhaite définir (voir MCP23S17_itControlType_e)
 */
void BSP_MCP23S17_setITControl_onPin(MCP23S17_port_e port, MCP23S17_pin_e pin, MCP23S17_itControlType_e type)
{
    // On récupère les états des contrôles d'interruption du port souhaité
    uint8_t value = MCP23S17_readSPI(MCP23S17_INTCON + port);

    // On modifie la méthode de contrôle du pin souhaité
...
/**
 * @brief Fonction qui permet de définir le contrôle d'interruption sur tous les pins d'un port
 * @param port : MCP23S17_PORT_A ou MCP23S17_PORT_B
 * @param type : type de contrôle (voir MCP23S17_itControlType_e)
 */
...
{
	// On récupère les états des GPIO du port
