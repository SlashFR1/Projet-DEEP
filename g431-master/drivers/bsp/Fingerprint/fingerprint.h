...
{
	uint16_t  pageid;
	char nom_utilisateur[32]; // stockage du nom de l'utilisateur
	bool is_occupied; // si le slot est occupé ou non
}fingerprint_page_t;
...
void send_command_withparameters(uint16_t address, uint8_t command, uint8_t* data, uint16_t lenght);
...
void send_delete(uint32_t addr, uint16_t pageid, uint16_t count);

void fingerprint_set_mode_save_new_finger(bool enable);
