...
void fingerprint_state_machine(void){
	typedef enum
	 ...
	 static state_e previous_state = INIT; //permet de sauvegarder l'�tat pr�c�dent
	 bool entrance;
	 entrance = (state!=previous_state)?true:false;
	 //le bool�en entrance est vrai si on vient juste de changer d'�tat !
	 previous_state = state; //l'�tat courant est sauvegard�, ce sera le prochain "�tat pr�c�dent"
			  ...
			  case SEARCH:
			 	...
			 	case STORE:
			 				 if(entrance)
			 				 {
			 						...
			 						printf("Demande d'enregistrement � la pageid = %d\n",pageid);
			 						pageid++;
			 						t = 8000;
			 				 }
			 				 if(flag_ack)
